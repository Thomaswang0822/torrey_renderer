 ...
 * @return ray 
 */
inline ray refract_ray(ray& rayIn, Vector3 outNormal, Vector3& hitPt, Real eta_ratio) {
    Real cos_theta = dot(-rayIn.dir, outNormal);
    Vector3 r_out_perp = eta_ratio * (rayIn.dir + cos_theta * outNormal);
    Vector3 r_out_parallel = -sqrt(fabs(1.f - length_squared(r_out_perp))) * outNormal;
    return ray(
        hitPt,
...
inline Real reflectance(Real cosine, Real eta_ratio) {
