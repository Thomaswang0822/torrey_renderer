...
struct ImageTexture {
     ...
     * 
     */
    Vector3 blerp_color(Real u, Real v) const {
        // null guardian
        if (img3.data.size() == 0) {
        ...
        // convert
        Real x = img3.width  * modulo(uscale * u + uoffset, 1.f) - 1e-9;
        Real y = img3.height * modulo(vscale * v + voffset, 1.f) - 1e-9;
 ...
 * @return Vector3 
 */
inline Vector3 eval_RGB(const Color& refl, const Real u, const Real v) {
...
inline Real compute_SchlickFresnel(Real F0, Real cos_theta) {
...
inline Vector3 mirror_SchlickFresnel_color(const Color& refl, 
        const Real u, const Real v, Real cos_theta) {
    Vector3 F0 = eval_RGB(refl, u, v);
    return F0 + (-F0 + 1.f) * pow(1.f - cos_theta, 5.f);
}
...
struct Phong {
    ...
    Vector3 compute_BRDF(const Real& cosTerm, Hit_Record& rec) {
        ...
        return Ks * (exponent + 1.f) * c_INVTWOPI * pow(std::max(cosTerm, 0.f), exponent);
    }

    Real compute_PDF(const Real& cosTerm) {
        return (exponent + 1.f) * c_INVTWOPI * pow(cosTerm, exponent);
    }
};
...
struct BlinnPhong {
    ...
    Vector3 get_Fh(const Vector3& h, const Vector3& out_dir, Hit_Record& rec) {
        Vector3 Ks = eval_RGB(reflectance, rec.u, rec.v);
        return Ks + (1.f - Ks) * pow(1.f-dot(h, out_dir), 5.f);
    }
    ...
    Real compute_PDF(const Vector3& h, const Vector3& out_dir, Hit_Record& rec) {
        return (exponent + 1.f) * pow(dot(rec.normal, h), exponent) *
...
struct Microfacet {
            ...
            const Vector3& out_dir, Hit_Record& rec) 
    {
        ...
        Real D = (exponent + 2.f) * c_INVTWOPI * pow(dot(rec.normal, h), exponent);
        // put together
        return Fh * D * G / (4.f * dot(rec.normal, in_dir));
    }
    ...
    Real compute_G1(const Vector3& h, const Vector3& dir, Hit_Record& rec) {
        if (dot(dir, h) <= 0) { return 0.f; }
        Real cos_theta = dot(dir, rec.normal);
        // assert(cos_theta > 1e-6);
        // cot = 1 / tan = cos / sin
        Real cotan_theta = cos_theta / sqrt(1.f - cos_theta * cos_theta);  
        Real a = sqrt(0.5 * exponent + 1.f) * cotan_theta;

        if (a >= 1.6) { return 1.f; }

        return (3.535 * a + 2.181 * a * a) / (1.f + 2.276 * a + 2.577 * a * a);
    }
            ...
            const Vector3& out_dir, Hit_Record& rec) 
    {
        ...
        // exponent = 2/alpha^2 - 2
        Real alpha_sq = 2.f / (exponent + 2.f);
    ...
    Real compute_G1_GGX(const Vector3& h, const Vector3& dir, Hit_Record& rec, const Real& alpha_sq) {
        ...
        Real tan_sq = (1.f - cos_theta_sq) / cos_theta_sq;
        // plug in formula (34)
        return 2.f / (1.f + sqrt(1.f + alpha_sq * tan_sq));
    }
    ...
    Real compute_PDF_GGX(const Vector3& h, const Vector3& out_dir, Hit_Record& rec) {
        // just D(m) * <m, n>
        Real alpha_sq = 2.f / (exponent + 2.f);
        return compute_D_GGX(h, rec, alpha_sq) * dot(h, rec.normal) / (4.f * dot(out_dir, h));
    }
